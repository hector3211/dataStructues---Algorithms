...
function binarySearchFunction(haystack, needle) {
  ...
  while (lo < hi) {
    const middle = Math.floor(lo + (hi - lo) / 2); // = 5 from our list
    const v = haystack[middle]; // this will be index 5 , number 6
    if (v === needle) {
      // if index 5,number 6 = needle return true
      return console.log(true);
    } else if (v > needle) {
      // if number 6 Greater than needle
      hi = middle; // we 1/2 our search and hi becomes index 5 instead of 10
    } else {
      // if number 6 less than needle
      lo = middle + 1; // our lo variable becomes index 6,number 7
    }
  }
